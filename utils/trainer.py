...
class Trainer():
	...
	def train(self, train_losses, train_set):
		...
		for i, (x, y, x_day,y_day) in enumerate(train_set):
			...
			if (self.lilw):
				output = self.model(x, original_x = x)[:,:,-1:,:,:]
			else:
				output = self.model(x)[:,:,-1:,:,:]
			#Disregard undefined pixels
			output = output*self.mask_land
			print("______________________________________________________output[:,:,0,:,:]",output[:,:,0,:,:]," y[:,:,0,:,:]", y[:,:,0,:,:])
			print("_________output[:,:,0,:,:] y[:,:,0,:,:].shape",output[:,:,0,:,:].shape,y[:,:,0,:,:].shape)
	...
	def evaluate(self, val_losses):
		...
		with torch.no_grad():
			for i, (x, y,x_day, y_day) in enumerate(self.val_data):
				print("____________________________________________________i %s x %s y %s of %s y_day %s" % (i,x.shape,y.shape,len(self.val_data),y_day) )
				x,y = x.to(self.device), y.to(self.device)
				output = self.model(x)
				print("____________________x y out",x,y,output)
				#Disregard undefined pixels
				output = output * self.mask_land
				loss = self.criterion(output, y)
	...
	def test_model(self):
		...
		for i, (x, x_border, y, y_border, x_day,y_day) in enumerate(self.test_data):
			...
			with torch.no_grad():
				...
				print("os.getcwd()",os.getcwd(),"y_day[0][0",y_day[0][0],"self.forecast_date",self.forecast_date)
				if y_day[0][0] == self.forecast_date:
					print("write forecast")
					self.write_asc(os.getcwd()+'/prediction_s2s__medianmodel_'+y_day[0][0]+'_7thday.asc', output[0,0,6,:,:].cpu().numpy())
					self.write_asc(os.getcwd()+'/prediction_s2s__medianmodel_'+y_day[0][0]+'_6thday.asc', output[0,0,5,:,:].cpu().numpy())
					self.write_asc(os.getcwd()+'/prediction_s2s__medianmodel_'+y_day[0][0]+'_5thday.asc', output[0,0,4,:,:].cpu().numpy())
					self.write_asc(os.getcwd()+'/prediction_s2s__medianmodel_'+y_day[0][0]+'_4thday.asc', output[0,0,3,:,:].cpu().numpy())
					self.write_asc(os.getcwd()+'/prediction_s2s__medianmodel_'+y_day[0][0]+'_3rdday.asc', output[0,0,2,:,:].cpu().numpy())
					self.write_asc(os.getcwd()+'/prediction_s2s__medianmodel_'+y_day[0][0]+'_2ndday.asc', output[0,0,1,:,:].cpu().numpy())
					self.write_asc(os.getcwd()+'/prediction_s2s__medianmodel_'+y_day[0][0]+'_1stday.asc', output[0,0,0,:,:].cpu().numpy())

				output = output[:,:,-1,:,:] * self.mask_land

				print("output.shape %s self.mask_land %s y %s" % (output.shape, self.mask_land.shape, y.shape) )
				print("y_day[0][0] %s self.mask_land %s " % (y_day[0][0], 1) )
				#self.write_asc('prediction_s2s__medianmodel_'+y_day[0][0]+'.asc', output[0,0,0,:,:].cpu().numpy())
				#self.write_asc('actual_s2s__medianmodel_'+y_day[0][0]+'.asc', y[0,0,0,:,:].cpu().numpy())
